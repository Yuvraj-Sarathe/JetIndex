    ...
    <div className="bg-card border border-ink-border rounded-xl p-5 shadow-sm">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm font-mono text-xs">
          <thead>
            <tr className="border-b border-ink-border text-ink-faint uppercase text-[11px]">
          ...
          <tbody className="divide-y divide-ink-border/40">
            {indices.map((idx) => {
              ...
              return (
                <tr key={idx.name} className={`hover:bg-card-hover transition-colors ${isPublished ? 'bg-accent-lime/10' : ''}`}>
                  <td className="py-2.5 px-3 font-sans font-medium text-white flex items-center gap-2">
                    {idx.name}
                    {isPublished && (
                      <span className="text-[10px] font-mono font-bold px-2 py-0.2 rounded bg-accent-lime text-ink-night uppercase">
                        Published Standard
                      </span>
                    )}
                  </td>
                  <td className="py-2.5 px-3 text-right font-bold text-accent-lime">{idx.value.toFixed(2)}</td>
                  <td className="py-2.5 px-3 text-ink-muted hidden md:table-cell font-sans">{idx.description}</td>
                  <td className="py-2.5 px-3 text-accent-pink hidden md:table-cell font-sans">{idx.bias}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      {/* Substitution Bias */}
      <div className="mt-4 p-3 bg-card-elevated border border-ink-border/80 rounded-xl flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-accent-cyan font-medium text-xs">Substitution Bias:</span>
